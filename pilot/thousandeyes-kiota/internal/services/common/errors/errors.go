...
func hintFor(status int, op Operation) string {
	...
	case http.StatusUnauthorized:
		return "The bearer token was rejected. A bearer token is issued out of band and the " +
			"provider never refreshes one, so this usually means it was revoked or is " +
			"malformed rather than that it expired."
	...
	case http.StatusBadRequest, http.StatusUnprocessableEntity:
		return "The API rejected the request body. A validation error often omits the " +
			"offending field, so compare the configuration against the ThousandEyes " +
