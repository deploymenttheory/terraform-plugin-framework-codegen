...
func Handle(diags *diag.Diagnostics, resourceType string, op Operation, err error) {
	...
	if detail == "" || detail == "error status code received from the API" {
		...
		// model usually still holds the API's words -- an API answering with
		// RFC 7807 problem documents carries a title and detail that kiota does
		// not map into the message -- so quote those before conceding there is
		// nothing.
	...
	msg := fmt.Sprintf("The Jamf Pro API returned %d %s.\n\n%s",
...
func hintFor(status int, op Operation) string {
	...
	case http.StatusUnauthorized:
		return "The access token was rejected. The provider obtains one with the " +
			"client-credentials grant and renews it before expiry, so this usually means the " +
			"client registration was revoked or its secret rotated rather than that the token " +
			"went stale."

	case http.StatusForbidden:
		return "The credentials authenticated but lack permission for this operation, or the " +
			"account group they are scoped to does not contain the object. Check the caller's " +
			"role and the provider's account_group_id."

	case http.StatusNotFound:
		if op == OpDelete {
			return "The object was already gone. This is usually benign."
		}
		return "The object does not exist in the account group the credentials are scoped to. " +
	...
	case http.StatusBadRequest, http.StatusUnprocessableEntity:
		return "The API rejected the request body. A validation error often omits the " +
			"offending field, so compare the configuration against the Jamf Pro " +
