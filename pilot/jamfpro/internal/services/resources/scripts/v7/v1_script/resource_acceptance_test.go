...
import (
	...
	"github.com/deploymenttheory/terraform-plugin-framework-codegen/pilot/jamfpro/internal/services/resources/scripts/v7/v1_script"
)

// The resource under test, and the helper that asks the API whether it is really there.
const address = "jamfpro_v1_script.test"

var testResource = v1_script.V1ScriptTestResource{}

// TestAccResourceV1Script_01_Lifecycle exercises the whole lifecycle against a live API.
//
...
func TestAccResourceV1Script_01_Lifecycle(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: destroy.CheckDestroyed(testResource, "jamfpro_v1_script", 20*time.Second),
		Steps: []resource.TestStep{
			...
			// Update in place, to a configuration that sets the optional attributes too.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					check.That(address).ExistsInAPI(testResource),
					check.That(address).Key("id").Exists(),
					check.That(address).Key("category_name").HasValue("tfacc-category-name"),
					check.That(address).Key("info").HasValue("tfacc-info"),
					check.That(address).Key("name").HasValue("tfacc-name"),
					check.That(address).Key("notes").HasValue("tfacc-notes"),
					check.That(address).Key("os_requirements").HasValue("tfacc-os-requirements"),
					check.That(address).Key("parameter10").HasValue("tfacc-parameter10"),
					check.That(address).Key("parameter11").HasValue("tfacc-parameter11"),
					check.That(address).Key("parameter4").HasValue("tfacc-parameter4"),
					check.That(address).Key("parameter5").HasValue("tfacc-parameter5"),
					check.That(address).Key("parameter6").HasValue("tfacc-parameter6"),
					check.That(address).Key("parameter7").HasValue("tfacc-parameter7"),
					check.That(address).Key("parameter8").HasValue("tfacc-parameter8"),
					check.That(address).Key("parameter9").HasValue("tfacc-parameter9"),
					check.That(address).Key("priority").HasValue("tfacc-priority"),
