...
import (
	...
	"github.com/deploymenttheory/terraform-plugin-framework-codegen/pilot/jamfpro/internal/services/resources/distribution_point/v7/v1_distribution_point"
)

// The resource under test, and the helper that asks the API whether it is really there.
const address = "jamfpro_v1_distribution_point.test"

var testResource = v1_distribution_point.V1DistributionPointTestResource{}

// TestAccResourceV1DistributionPoint_01_Lifecycle exercises the whole lifecycle against a live API.
//
...
func TestAccResourceV1DistributionPoint_01_Lifecycle(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: destroy.CheckDestroyed(testResource, "jamfpro_v1_distribution_point", 20*time.Second),
		Steps: []resource.TestStep{
			// Create, from the smallest configuration the schema permits.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					check.That(address).Key("file_sharing_connection_type").HasValue("tfacc-file-sharing-connection-type"),
					check.That(address).Key("name").HasValue("tfacc-name"),
					check.That(address).Key("server_name").HasValue("tfacc-server-name"),
				),
			},
			...
			// Update in place, to a configuration that sets the optional attributes too.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					check.That(address).ExistsInAPI(testResource),
					check.That(address).Key("id").Exists(),
					check.That(address).Key("enable_load_balancing").HasValue("true"),
					check.That(address).Key("file_sharing_connection_type").HasValue("tfacc-file-sharing-connection-type"),
					check.That(address).Key("https_context").HasValue("tfacc-https-context"),
					check.That(address).Key("https_enabled").HasValue("true"),
					check.That(address).Key("https_port").HasValue("1"),
					check.That(address).Key("https_security_type").HasValue("tfacc-https-security-type"),
					check.That(address).Key("local_path_to_share").HasValue("tfacc-local-path-to-share"),
					check.That(address).Key("name").HasValue("tfacc-name"),
					check.That(address).Key("port").HasValue("1"),
					check.That(address).Key("principal").HasValue("true"),
					check.That(address).Key("server_name").HasValue("tfacc-server-name"),
					check.That(address).Key("share_name").HasValue("tfacc-share-name"),
					check.That(address).Key("workgroup").HasValue("tfacc-workgroup"),
				),
			},
		},
	})
