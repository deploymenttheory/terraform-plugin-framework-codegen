...
import (
	...
	"github.com/deploymenttheory/terraform-plugin-framework-codegen/pilot/jamfpro/internal/services/resources/buildings/v7/v1_building"
)

// The resource under test, and the helper that asks the API whether it is really there.
const address = "jamfpro_v1_building.test"

var testResource = v1_building.V1BuildingTestResource{}

// TestAccResourceV1Building_01_Lifecycle exercises the whole lifecycle against a live API.
//
...
func TestAccResourceV1Building_01_Lifecycle(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: destroy.CheckDestroyed(testResource, "jamfpro_v1_building", 20*time.Second),
		Steps: []resource.TestStep{
			...
			// Update in place, to a configuration that sets the optional attributes too.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					check.That(address).ExistsInAPI(testResource),
					check.That(address).Key("id").Exists(),
					check.That(address).Key("city").HasValue("tfacc-city"),
					check.That(address).Key("country").HasValue("tfacc-country"),
					check.That(address).Key("name").HasValue("tfacc-name"),
					check.That(address).Key("state_province").HasValue("tfacc-state-province"),
					check.That(address).Key("street_address1").HasValue("tfacc-street-address1"),
					check.That(address).Key("street_address2").HasValue("tfacc-street-address2"),
					check.That(address).Key("zip_postal_code").HasValue("tfacc-zip-postal-code"),
				),
			},
		},
	})
