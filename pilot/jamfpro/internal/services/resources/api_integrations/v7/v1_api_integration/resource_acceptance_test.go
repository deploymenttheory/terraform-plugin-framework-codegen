...
import (
	...
	"github.com/deploymenttheory/terraform-plugin-framework-codegen/pilot/jamfpro/internal/services/resources/api_integrations/v7/v1_api_integration"
)

// The resource under test, and the helper that asks the API whether it is really there.
const address = "jamfpro_v1_api_integration.test"

var testResource = v1_api_integration.V1APIIntegrationTestResource{}

// TestAccResourceV1APIIntegration_01_Lifecycle exercises the whole lifecycle against a live API.
//
...
func TestAccResourceV1APIIntegration_01_Lifecycle(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: destroy.CheckDestroyed(testResource, "jamfpro_v1_api_integration", 20*time.Second),
		Steps: []resource.TestStep{
			// Create, from the smallest configuration the schema permits.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					check.That(address).Key("display_name").HasValue("tfacc-display-name"),
			...
			// Update in place, to a configuration that sets the optional attributes too.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					check.That(address).ExistsInAPI(testResource),
					check.That(address).Key("id").Exists(),
					check.That(address).Key("access_token_lifetime_seconds").HasValue("1"),
					check.That(address).Key("display_name").HasValue("tfacc-display-name"),
					check.That(address).Key("enabled").HasValue("true"),
				),
			},
