...
import (
	...
	"github.com/deploymenttheory/terraform-plugin-framework-codegen/pilot/jamfpro/internal/services/resources/self_service_branding_ios/v7/v1_self_service_branding_io"
)

// The resource under test, and the helper that asks the API whether it is really there.
const address = "jamfpro_v1_self_service_branding_io.test"

var testResource = v1_self_service_branding_io.V1SelfServiceBrandingIoTestResource{}

// TestAccResourceV1SelfServiceBrandingIo_01_Lifecycle exercises the whole lifecycle against a live API.
//
...
func TestAccResourceV1SelfServiceBrandingIo_01_Lifecycle(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: destroy.CheckDestroyed(testResource, "jamfpro_v1_self_service_branding_io", 20*time.Second),
		Steps: []resource.TestStep{
			// Create, from the smallest configuration the schema permits.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					check.That(address).Key("branding_name").HasValue("tfacc-branding-name"),
					check.That(address).Key("branding_name_color_code").HasValue("tfacc-branding-name-color-code"),
					check.That(address).Key("header_background_color_code").HasValue("tfacc-header-background-color-code"),
					check.That(address).Key("menu_icon_color_code").HasValue("tfacc-menu-icon-color-code"),
					check.That(address).Key("status_bar_text_color").HasValue("tfacc-status-bar-text-color"),
				),
			},
			...
			// Update in place, to a configuration that sets the optional attributes too.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					check.That(address).Key("branding_name_color_code").HasValue("tfacc-branding-name-color-code"),
					check.That(address).Key("header_background_color_code").HasValue("tfacc-header-background-color-code"),
					check.That(address).Key("icon_id").HasValue("1"),
					check.That(address).Key("menu_icon_color_code").HasValue("tfacc-menu-icon-color-code"),
					check.That(address).Key("status_bar_text_color").HasValue("tfacc-status-bar-text-color"),
				),
			},
