...
// drift silently. The value matters more than it looks: hcspec.Validate switches on
...
const SpecVersion = hcspec.Version0_1
...
func Marshal(s hcspec.Specification) ([]byte, error) {
...
//
// hcspec.Parse already does both, in that order; this wraps it so the failure
...
func Parse(ctx context.Context, data []byte) (hcspec.Specification, error) {
	s, err := hcspec.Parse(ctx, data)
	if err != nil {
		return hcspec.Specification{}, fmt.Errorf("%w: %w", ErrInvalidSpec, err)
	}
...
func Validate(ctx context.Context, data []byte) error {
	if err := hcspec.Validate(ctx, data); err != nil {
