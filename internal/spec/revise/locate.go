...
func (l *locator) nodeAt(ptr string) *yaml.Node {
	node := l.top
	if ptr == "" {
		return node
	}
...
func (l *locator) followSchemaRefs(node *yaml.Node, ptr string) (*yaml.Node, string, bool) {
	for hops := 0; hops < 32; hops++ {
		ref := mapValue(node, "$ref")
		if ref == nil {
			return node, ptr, node != nil
		}
...
func (l *locator) requestSchema(op *specmodel.Op) (*yaml.Node, string, bool) {
	...
	if ref := mapValue(body, "$ref"); ref != nil {
		...
		ptr = "/components/requestBodies/" + escapeToken(name)
		body = l.nodeAt(ptr)
	}
	return l.contentSchema(body, ptr)
}
...
func (l *locator) responseSchema(op *specmodel.Op) (*yaml.Node, string, bool) {
	...
	for _, status := range statuses {
		...
		if strings.HasPrefix(status, "2") {
			if node, ptr, ok := l.responseContent(responses, opPtr, status); ok {
...
func (l *locator) responseContent(responses *yaml.Node, opPtr, status string) (*yaml.Node, string, bool) {
	...
	ptr := opPtr + "/responses/" + escapeToken(status)
	if ref := mapValue(node, "$ref"); ref != nil {
		name, ok := strings.CutPrefix(ref.Value, "#/components/responses/")
		if !ok {
			return nil, "", false
		}
		ptr = "/components/responses/" + escapeToken(name)
		node = l.nodeAt(ptr)
	}
	return l.contentSchema(node, ptr)
}
...
func (l *locator) contentSchema(node *yaml.Node, ptr string) (*yaml.Node, string, bool) {
	...
	for _, mt := range mediaTypes {
		if schema := mapValue(mapValue(content, mt), "schema"); schema != nil {
			return l.followSchemaRefs(schema, ptr+"/content/"+escapeToken(mt)+"/schema")
		}
	}
	return nil, "", false
}
...
func (l *locator) findPropertyGuarded(node *yaml.Node, ptr, name string, seen map[string]bool) (propSite, bool) {
	...
	if allOf := mapValue(node, "allOf"); allOf != nil && allOf.Kind == yaml.SequenceNode {
		for i, branch := range allOf.Content {
			if site, ok := l.findPropertyGuarded(branch, ptr+"/allOf/"+strconv.Itoa(i), name, seen); ok {
				return site, true
			}
		}
	}
...
func (l *locator) requiredHas(node *yaml.Node, ptr, name string) bool {
	return l.walkSchema(node, ptr, func(n *yaml.Node) bool {
		req := mapValue(n, "required")
...
func (l *locator) readOnlyDeclared(node *yaml.Node, ptr string) bool {
	return l.walkSchema(node, ptr, func(n *yaml.Node) bool {
...
func (l *locator) extensionNode(node *yaml.Node, ptr, key string) *yaml.Node {
	var found *yaml.Node
	l.walkSchema(node, ptr, func(n *yaml.Node) bool {
...
func (l *locator) enumSite(node *yaml.Node, ptr string) (*yaml.Node, string, bool) {
	var siteNode *yaml.Node
	var sitePtr string
	l.walkSchemaWithPtr(node, ptr, map[string]bool{}, func(n *yaml.Node, p string) bool {
...
func (l *locator) walkSchema(node *yaml.Node, ptr string, visit func(*yaml.Node) bool) bool {
	return l.walkSchemaWithPtr(node, ptr, map[string]bool{}, func(n *yaml.Node, _ string) bool {
		return visit(n)
	})
}

func (l *locator) walkSchemaWithPtr(node *yaml.Node, ptr string, seen map[string]bool, visit func(*yaml.Node, string) bool) bool {
	if node == nil || seen[ptr] {
		return false
	}
	seen[ptr] = true
	if visit(node, ptr) {
	...
	resolved, resolvedPtr, ok := l.followSchemaRefs(node, ptr)
	if ok && resolved != node {
	...
	if allOf := mapValue(node, "allOf"); allOf != nil && allOf.Kind == yaml.SequenceNode {
		for i, branch := range allOf.Content {
			if l.walkSchemaWithPtr(branch, ptr+"/allOf/"+strconv.Itoa(i), seen, visit) {
				return true
			}
		}
	}
