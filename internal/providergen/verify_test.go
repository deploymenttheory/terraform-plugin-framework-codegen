...
// TestUnit_Verify_ToleratesTheToolchainFinalizedFiles does what tree
// verification's `go mod tidy` does after install — rewrites go.mod, writes a
...
// digests, not to a regeneration that never runs the toolchain, and the
// SDK binds in place against the pair.
