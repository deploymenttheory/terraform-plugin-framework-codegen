...
func TestUnit_Generate_FileSHA256(t *testing.T) {
	t.Parallel()
...
// TestUnit_Generate_WriteReportsUnreadableTargets covers the path where the target
// exists but cannot be inspected, which must fail rather than overwrite blindly.
func TestUnit_Generate_WriteReportsUnreadableTargets(t *testing.T) {
	t.Parallel()
	...
	plan, err := gen.Build(bp, BuildOptions{BlueprintPath: "b"})
	if err != nil {
...
func TestUnit_Generate_RenderFileRejectsAnUnknownTemplate(t *testing.T) {
	t.Parallel()
...
func TestUnit_Generate_NumberLines(t *testing.T) {
	t.Parallel()
