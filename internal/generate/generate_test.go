...
// TestUnit_Generate_LangVersionIsAccepted pins the gofumpt language version.
//
...
func TestUnit_Generate_LangVersionIsAccepted(t *testing.T) {
	t.Parallel()
...
// TestUnit_Generate_IsDeterministic is the property the whole drift gate rests on.
//
...
func TestUnit_Generate_IsDeterministic(t *testing.T) {
	t.Parallel()
	...
	first, err := gen.Build(bp, BuildOptions{BlueprintPath: "blueprints/thousandeyes"})
	if err != nil {
	...
	for i := 0; i < 25; i++ {
		again, err := gen.Build(bp, BuildOptions{BlueprintPath: "blueprints/thousandeyes"})
		if err != nil {
			t.Fatalf("Build: %v", err)
		}
...
// TestUnit_Generate_CarriesNoTimestampOrVersion guards the two values most likely to
// be added later by someone trying to be helpful. Either would make every
// regeneration a diff.
func TestUnit_Generate_CarriesNoTimestampOrVersion(t *testing.T) {
	t.Parallel()
	...
	plan, err := gen.Build(bp, BuildOptions{BlueprintPath: "blueprints/thousandeyes"})
	if err != nil {
		t.Fatalf("Build: %v", err)
	}
...
// TestUnit_Generate_EveryFileIsMarkedGenerated matters because overwrite protection
// keys on the marker: a generated file lacking it could never be regenerated
// without -force.
func TestUnit_Generate_EveryFileIsMarkedGenerated(t *testing.T) {
	t.Parallel()
	...
	for _, f := range plan.Files {
		...
		if strings.HasSuffix(f.Path, ".tf") || strings.HasSuffix(f.Path, ".sh") {
			prefix = "# Code generated by"
		}
...
// TestUnit_Generate_RefusesToOverwriteHandWrittenFiles is the guard that stops a
// mistyped -out from destroying somebody's work with no way to recover it.
func TestUnit_Generate_RefusesToOverwriteHandWrittenFiles(t *testing.T) {
	t.Parallel()
...
// TestUnit_Generate_WriteIsIdempotent confirms a second write reports everything as
...
func TestUnit_Generate_WriteIsIdempotent(t *testing.T) {
	t.Parallel()
	...
	if got := len(second.Unchanged); got != len(plan.Files) {
		t.Errorf("second write reported %d unchanged, want %d", got, len(plan.Files))
	}
}

// TestUnit_Generate_OnlyRestrictsToOneResource covers the flag used while iterating on
// a template.
func TestUnit_Generate_OnlyRestrictsToOneResource(t *testing.T) {
	t.Parallel()
	...
	plan, err := gen.Build(bp, BuildOptions{BlueprintPath: "b", Only: "tag"})
	if err != nil {
		t.Fatalf("Build: %v", err)
	}
	...
	if _, err := gen.Build(bp, BuildOptions{BlueprintPath: "b", Only: "nonexistent"}); err != nil {
		t.Fatalf("Build with an unmatched -only should succeed and produce nothing: %v", err)
	}
}

// TestUnit_Generate_FormatErrorIncludesNumberedSource exists because a template bug
...
func TestUnit_Generate_FormatErrorIncludesNumberedSource(t *testing.T) {
	t.Parallel()
...
// TestUnit_Generate_AllowedValuesBecomeAValidator is the inverse of the test that used to be
// here.
//
// TestUnit_Generate_EnumValuesDoNotReachGeneratedCode asserted that AttrType.Enum reached no
...
func TestUnit_Generate_AllowedValuesBecomeAValidator(t *testing.T) {
	t.Parallel()
...
// TestUnit_Generate_ASplitUpdateBodyGetsItsOwnConstructFunction.
//
// An SDK that gives update its own request type needs a second construct function over
// the same assignment list, and the generated update must call it -- calling
// constructResource there would send create's type to a method that takes update's,
// which does not compile in someone else's provider.
func TestUnit_Generate_ASplitUpdateBodyGetsItsOwnConstructFunction(t *testing.T) {
	t.Parallel()
	...
	for i := range bp.Resources {
		...
		bp.Resources[i].Binding.Body.UpdateRequestType = "tags.TagUpdate"
		bp.Resources[i].Binding.Body.UpdateConstructorExpr = "&tags.TagUpdate{}"
	}
	...
	var construct, crud string
	for _, f := range plan.Files {
		switch {
		case strings.HasSuffix(f.Path, "resources/tags/v7/tag/construct.go"):
			construct = string(f.Content)
		case strings.HasSuffix(f.Path, "resources/tags/v7/tag/crud.go"):
			crud = string(f.Content)
		}
	}
	if construct == "" || crud == "" {
		t.Fatal("the construct or crud file was not emitted")
	}

	if !strings.Contains(construct, "func constructResourceUpdate") {
		t.Error("a split update body should emit constructResourceUpdate")
	}
	if !strings.Contains(construct, "*tags.TagUpdate") {
		t.Error("the update construct function should build the update type")
	}
	if !strings.Contains(crud, "constructResourceUpdate(ctx, &plan)") {
		t.Error("the generated update should call constructResourceUpdate")
	}
	// Create keeps the original body; only update switches.
	if !strings.Contains(crud, "constructResource(ctx, &plan)") {
		t.Error("the generated create should still call constructResource")
	}

	// And without the split, no second function appears -- the common shape stays
	// exactly as it was.
	plain, err := gen.Build(pilotBlueprint(t), BuildOptions{BlueprintPath: "blueprints/thousandeyes"})
	if err != nil {
		t.Fatalf("Build (plain): %v", err)
	}
	for _, f := range plain.Files {
		if strings.HasSuffix(f.Path, "resources/tags/v7/tag/construct.go") &&
			strings.Contains(string(f.Content), "constructResourceUpdate") {
			t.Error("an unsplit body must not grow an update construct function")
		}
	}
}

// TestUnit_Generate_ACuratedFixtureValueRidesWithItsDataBlock.
//
...
func TestUnit_Generate_ACuratedFixtureValueRidesWithItsDataBlock(t *testing.T) {
	t.Parallel()
...
// TestUnit_Generate_AGatedResourceSkipsItsTestsByName.
//
...
func TestUnit_Generate_AGatedResourceSkipsItsTestsByName(t *testing.T) {
	t.Parallel()
...
// TestUnit_Generate_APurelyComputedAttributeGetsNoValidator.
//
...
func TestUnit_Generate_APurelyComputedAttributeGetsNoValidator(t *testing.T) {
	t.Parallel()
...
// TestUnit_Generate_DataSourceProducesItsFileSet pins the per-data-source file split.
//
...
func TestUnit_Generate_DataSourceProducesItsFileSet(t *testing.T) {
	t.Parallel()
...
// TestUnit_Generate_DataSourcesRegisterInTheProvider checks the generated registry, which is
// what makes the provider serve them at all.
func TestUnit_Generate_DataSourcesRegisterInTheProvider(t *testing.T) {
	t.Parallel()
...
// firstGenerated returns the plan's first file, failing rather than indexing an
// empty plan.
func firstGenerated(t *testing.T, plan Fileset) File {
	t.Helper()
