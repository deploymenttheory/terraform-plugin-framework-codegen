...
func TestUnit_CLI_Interop_ExportMatchesTheCommittedSpec(t *testing.T) {
	...
	if err := runSpecGroup([]string{"export", "-blueprint", blueprintDir(), "-out", out, "-q"}); err != nil {
...
func TestUnit_CLI_Interop_TheCommittedSpecIsValid(t *testing.T) {
	...
	if len(doc.Resources) != 23 || doc.Resources[0].Name != "account_group" {
		t.Fatalf("got %d resources starting %q, want 23 starting with account_group",
...
func TestUnit_CLI_Interop_ExportToStdout(t *testing.T) {
	...
	if err := runSpecGroup([]string{"export", "-blueprint", blueprintDir(), "-q"}); err != nil {
...
func TestUnit_CLI_Interop_Only(t *testing.T) {
	...
	if err := runSpecGroup([]string{"export", "-blueprint", blueprintDir(), "-resource", "tag", "-out", out, "-q"}); err != nil {
	...
	err = runSpecGroup([]string{"export", "-blueprint", blueprintDir(), "-resource", "octopus", "-q"})
...
func TestUnit_CLI_Interop_ImportList(t *testing.T) {
	...
	// -out nor -provider.
	if err := runSpecGroup([]string{"import", "-spec", committedSpec(), "-dry-run"}); err != nil {
...
func TestUnit_CLI_Interop_ImportRejectsBadInput(t *testing.T) {
	...
	if err := runSpecGroup([]string{"import", "-spec", bad, "-provider", "x", "-out", t.TempDir(), "-q"}); err == nil {
		t.Error(`a document declaring "0.1.0" must be refused`)
	}

	if err := runSpecGroup([]string{
...
func TestUnit_CLI_Interop_HelpIsNotAnError(t *testing.T) {
	...
	for _, arg := range []string{"-h", "-help", "--help"} {
		if err := runSpecGroup([]string{arg}); err != nil {
...
func TestUnit_CLI_Interop_ExportReportsAMissingBlueprint(t *testing.T) {
	...
	err := runSpecGroup([]string{"export", "-blueprint", filepath.Join(t.TempDir(), "absent"), "-q"})
	if err == nil {
