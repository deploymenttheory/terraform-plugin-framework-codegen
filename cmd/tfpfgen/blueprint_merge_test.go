...
func TestUnit_CLI_Merge_CorroboratedDefaultApplies(t *testing.T) {
	...
	if err := runBlueprintMerge([]string{
...
func TestUnit_CLI_Merge_IsIdempotentThroughTheCLI(t *testing.T) {
	...
	if err := runBlueprintMerge(args); err != nil {
...
func TestUnit_CLI_Merge_AcceptConflictsSuppressesOnlyTheExitCode(t *testing.T) {
	...
	if err := runBlueprintMerge([]string{
		...
		"-strategy", "apply", "-allow-conflicts", "-q",
	}); err != nil {
		t.Errorf("-allow-conflicts should suppress the exit code: %v", err)
	}

	if after := readFile(t, filepath.Join(dir, "resources", "tag.blueprint.json")); after != before {
		t.Error("-allow-conflicts must not apply the conflicting change")
	}
}
...
func TestUnit_CLI_Merge_RejectsBadUsage(t *testing.T) {
	...
	for name, args := range tests {
		t.Run(name, func(t *testing.T) {
			...
			var ue *usageError
			if err := runBlueprintMerge(args); !errors.As(err, &ue) {
	...
	if err := runBlueprintMerge([]string{"-blueprint", blueprintDir(), "-facts", bad, "-q"}); err == nil {
