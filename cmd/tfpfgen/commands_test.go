...
func emitInto(t *testing.T) string {
	...
	out := t.TempDir()
	if err := runProviderGenerate([]string{"-blueprint", blueprintDir(), "-out", out}); err != nil {
		t.Fatalf("emit: %v", err)
	}
	return out
}
...
func TestUnit_CLI_Emit_WritesAndIsIdempotent(t *testing.T) {
	...
	if err := runProviderGenerate([]string{"-check", "-blueprint", blueprintDir(), "-out", out, "-summary", ""}); err != nil {
...
func TestUnit_CLI_Emit_DryRunAndListTouchNothing(t *testing.T) {
	...
	got := captureStdout(t, func() {
		if err := runProviderGenerate([]string{"-blueprint", blueprintDir(), "-out", out, "-dry-run"}); err != nil {
	...
	if err := runProviderGenerate([]string{"-blueprint", blueprintDir(), "-dry-run"}); err != nil {
		t.Errorf("list: %v", err)
	}
}
...
func TestUnit_CLI_Emit_OnlyLeavesTheManifestAlone(t *testing.T) {
	...
	if err := runProviderGenerate([]string{"-blueprint", blueprintDir(), "-out", out, "-resource", "tag"}); err != nil {
...
func TestUnit_CLI_Emit_UnmatchedOnlyIsAnError(t *testing.T) {
	...
	err := runProviderGenerate([]string{"-blueprint", blueprintDir(), "-out", t.TempDir(), "-resource", "nonexistent"})
...
func TestUnit_CLI_Verify_DetectsEachDriftClass(t *testing.T) {
	...
	t.Run("drifted", func(t *testing.T) {
		...
		err = runProviderGenerate([]string{"-check", "-blueprint", blueprintDir(), "-out", out, "-summary", ""})
		if err == nil {
...
func TestUnit_CLI_Verify_WritesAStepSummary(t *testing.T) {
	...
	if err := runProviderGenerate([]string{"-check", "-blueprint", blueprintDir(), "-out", out, "-summary", summary}); err == nil {
	...
	for _, want := range []string{"out of date", "tfpfgen provider generate"} {
...
func TestUnit_CLI_Verify_RequiredFlags(t *testing.T) {
	...
	for name, args := range map[string][]string{
		"no blueprint": {"-check"},
		"no out":       {"-check", "-blueprint", blueprintDir()},
	} {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			if err := runProviderGenerate(args); err == nil {
...
func TestUnit_CLI_Bindings(t *testing.T) {
	...
	t.Run("required flags", func(t *testing.T) {
		t.Parallel()
		if err := runBindingsCheck(nil); err == nil {
			t.Error("expected -blueprint to be required")
		}
		if err := runBindingsCheck([]string{"-blueprint", blueprintDir()}); err == nil {
...
func TestUnit_CLI_Ingest_MissingSnapshotIsReported(t *testing.T) {
	...
	err := runBlueprintDraft([]string{"-openapi-dir", t.TempDir(), "-dry-run"})
	if err == nil {
...
func TestUnit_CLI_Ingest_ReadsAnExplicitSpec(t *testing.T) {
	...
	captureStdout(t, func() {
		if err := runBlueprintDraft([]string{"-openapi", spec, "-tag", "tag", "-dry-run"}); err != nil {
...
func TestUnit_CLI_Emit_CleanRemovesOrphans(t *testing.T) {
	...
	if err := runProviderGenerate([]string{"-blueprint", blueprintDir(), "-out", out, "-clean"}); err != nil {
...
func TestUnit_CLI_Emit_RefusesToClobberHandWrittenFiles(t *testing.T) {
	...
	if err := runProviderGenerate([]string{"-blueprint", blueprintDir(), "-out", out, "-force"}); err != nil {
