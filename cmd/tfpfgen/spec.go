...
// because a run function that reads specVerbs while specVerbs is being
// initialised with that same function is an initialisation cycle the compiler
// rejects.
const (
	usageSpecExport = "spec export -blueprint DIR [-out FILE] [-resource KEY] [-strict] [-report FILE]"
	usageSpecImport = "spec import -spec FILE -out DIR -provider NAME [flags]"
)

// specVerbs is the spec group's verb table.
var specVerbs = []command{
	{
		name:    "export",
		summary: "write Provider Code Specification v0.1 JSON from blueprints",
		usage:   usageSpecExport,
		run:     runSpecExport,
	},
	{
		name:    "import",
		summary: "read Provider Code Specification v0.1 JSON into draft blueprints",
		usage:   usageSpecImport,
		run:     runSpecImport,
	},
}

func runSpecGroup(args []string) error {
	return runVerbs("spec", specVerbs, "", args)
}

func runSpecExport(args []string) error {
	fs, _ := newFlagSet("spec export", usageSpecExport)
	...
	var (
		...
		resource      = fs.String("resource", "", "restrict the export to one resource, by blueprint key")
	...
	// problems look like conversion bugs.
	...
	if *resource != "" {
		if err := keepOnly(&bp, *resource); err != nil {
			return err
		}
	}
...
func runSpecImport(args []string) error {
	fs, _ := newFlagSet("spec import", usageSpecImport)

	var (
		specPath = fs.String("spec", "", "Provider Code Specification v0.1 JSON to read (required)")
		...
		dryRun = fs.Bool("dry-run", false, "report what the document offers and write nothing")
	)
...
bindings against the SDK:
  ...
  go run ./cmd/tfpfgen bindings check -blueprint <dir> -module <provider dir>
...
// merge identifier every other subcommand's -resource already uses.
func keepOnly(bp *blueprint.Blueprint, key string) error {
	...
	return usagef("-resource %q matches no resource or data source key", key)
}
