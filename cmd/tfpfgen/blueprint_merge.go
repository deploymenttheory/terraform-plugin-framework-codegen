...
const usageBlueprintMerge = "blueprint merge -blueprint DIR -facts FILE [-strategy annotate|apply] " +
	"[-check] [-allow-conflicts] [-summary PATH]"

func runBlueprintMerge(args []string) error {
	fs, _ := newFlagSet("blueprint merge", usageBlueprintMerge)
	...
	var (
		...
		allowConflicts = fs.Bool("allow-conflicts", false,
	...
	if *adoptDir != "" {
		...
		if promoted > 0 {
			...
			result.Changes = append(result.Changes, merge.Change{
				Resource: "accFixture",
				What:     "scenario fixture values adopted as wire hints",
...
//
// -allow-conflicts suppresses the exit code only. It never applies anything: a flag that made
