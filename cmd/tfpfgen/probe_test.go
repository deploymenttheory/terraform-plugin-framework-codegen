...
func TestUnit_CLI_Probe_ListsTheCatalogue(t *testing.T) {
	...
	if err := runProbe([]string{"list", "-blueprint", blueprintDir(), "-resource", "tag"}); err != nil {
...
func TestUnit_CLI_Probe_ListNarrowsToOneProbe(t *testing.T) {
	...
	if err := runProbe([]string{
		"list", "-blueprint", blueprintDir(), "-resource", "tag", "-probe", "read.volatile",
	}); err != nil {
...
func TestUnit_CLI_Probe_MutationsOutsideRecordAreRefused(t *testing.T) {
	...
	for _, mode := range []string{"replay", "verify", "sweep"} {
		t.Run(mode, func(t *testing.T) {
			...
			err := runProbe([]string{
				mode, "-blueprint", blueprintDir(), "-resource", "tag",
				"-allow-mutations",
			})
...
func TestUnit_CLI_Probe_SweepNeedsCredentialsFromTheEnvironment(t *testing.T) {
	...
	err := runProbe([]string{"sweep", "-blueprint", blueprintDir(), "-resource", "tag"})
	if err == nil {
...
func TestUnit_CLI_Probe_ReplayNeedsACommittedCassette(t *testing.T) {
	...
	err := runProbe([]string{
		"replay", "-blueprint", blueprintDir(), "-resource", "tag",
		"-recordings", filepath.Join(t.TempDir(), "absent"),
	})
	if err == nil {
...
func TestUnit_CLI_Probe_RecordNeedsCredentialsFromTheEnvironment(t *testing.T) {
	...
	err := runProbe([]string{"record", "-blueprint", blueprintDir(), "-resource", "tag"})
...
func TestUnit_CLI_Probe_RejectsBadUsage(t *testing.T) {
	...
	tests := []struct {
		...
		{"no blueprint", nil},
		{"unknown verb", []string{"telepathy", "-blueprint", blueprintDir()}},
		{"unknown resource", []string{"list", "-blueprint", blueprintDir(), "-resource", "octopus"}},
		{"unknown probe", []string{"list", "-blueprint", blueprintDir(), "-probe", "read.telepathy"}},
		...
		{"rederive outside replay", []string{
			"verify", "-blueprint", blueprintDir(), "-resource", "tag", "-rederive",
		}},
		{"rederive with sweep", []string{
			"sweep", "-blueprint", blueprintDir(), "-resource", "tag", "-rederive",
		}},
	}
...
func TestUnit_CLI_Probe_ReportsAnUnprobeableResourceWithoutFailing(t *testing.T) {
	...
	if err := runProbe([]string{"list", "-blueprint", blueprintDir()}); err != nil {
...
//
// The verb that can change somebody's tenant has to be spelled out; the one that cannot is what
// you get by typing less. If this ever flips, a bare `probe -blueprint …` becomes a command
// that reaches for the network.
func TestUnit_CLI_Probe_DefaultVerbIsReplay(t *testing.T) {
	t.Parallel()

	if modeReplay == modeRecord {
		t.Fatal("replay and record cannot be the same verb")
	}

	// runProbe passes modeReplay as runVerbs' default; assert the wiring rather than
	// a constant, so this fails if somebody re-points the default at another verb.
	err := runProbe([]string{"-blueprint", filepath.Join(t.TempDir(), "absent")})
	if err == nil || strings.Contains(err.Error(), "needs a verb") {
		t.Errorf("a bare probe run must default to replay, got: %v", err)
	}
}
