// Command tfpfgen generates terraform-plugin-framework providers from an
// API specification plus recorded API behaviour.
//
//	tfpfgen blueprint draft -tag Tags -out blueprints/thousandeyes
//	tfpfgen probe replay -blueprint blueprints/thousandeyes
//	tfpfgen provider generate -blueprint blueprints/thousandeyes -out pilot/thousandeyes
//	tfpfgen provider generate -check -blueprint blueprints/thousandeyes -out pilot/thousandeyes
//
// The grammar is noun-group: the first word names the artefact, the second what
// to do to it. Each verb owns its own flag.FlagSet, so there is no global flag
// namespace for two subcommands to collide in, and argument parsing needs no
...
func main() {
	if err := run(os.Args[1:], os.Stderr); err != nil {
		...
		fmt.Fprintf(os.Stderr, "tfpfgen: %v\n", err)
