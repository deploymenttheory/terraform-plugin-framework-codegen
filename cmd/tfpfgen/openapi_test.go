...
func TestUnit_CLI_Specs_PinsAndThenRefreshesFromItsOwnRecord(t *testing.T) {
	...
	if err := runOpenAPIFetch([]string{"-url", srv.URL + "/api.yaml", "-out", root}); err != nil {
	...
	if err := runOpenAPIFetch([]string{"-out", root}); err != nil {
...
func TestUnit_CLI_Specs_RefusesABrokenDownload(t *testing.T) {
	...
	err := runOpenAPIFetch([]string{"-url", srv.URL, "-out", root})
...
func TestUnit_CLI_Specs_FirstPinNeedsAURL(t *testing.T) {
	...
	err := runOpenAPIFetch([]string{"-out", filepath.Join(t.TempDir(), "specs")})
